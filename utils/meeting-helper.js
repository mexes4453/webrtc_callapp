    ...
    .getAllMeetingUsers( meetingId, 
                         (error, results) => {
                            for (let i = 0; i < results.length; i++)
                            {
                                ...
                                meetingServer
                                .sockets
                                .connected[meetingUser.socketId]
                                .disconnect();
                            }
                         }
                       );
...
function addUser(socket, {meetingId, userId, name})
{
    let promise = new Promise(function (resolve, reject){
        meetingServices
        .getMeetingUser({ meetingId, userId },
                        (error, results) => {
                            if (!results)
                            {
                                var model = {
